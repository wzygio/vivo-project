...
- [x] 页面可独立发现并打开，无时间筛选框，窗口为上一自然月 1 日至当前日期；门户 `app/static/config.js` 注册。
...
### 2026-08-11 — 同步 AOI_RS 优化（样式 + By Lot 口径）

> *This was generated by AI during triage.*

应用户要求将 AOI_RS 的两轮优化同步至本报表（由 AOI_RS 开发会话执行）：
①图表样式——趋势图横轴去年份（07/W31/08-09）、趋势图仅保留检测片数柱图注（折线与 USL/UCL 不进图注）、By Lot/By Sheet 图注下移至 y=-0.5（底边距 200）不再遮挡竖排 ID；②By Lot 口径——由 Σtt_qty 改为 Lot 内平均每片（Σtt_qty ÷ 该 Lot 内 distinct sheet 数；分母取自 TT 测量表自身，每片必测故无除零场景），点线图新增 `y_col` 参数，标题/轴改为"平均每片 TT 个数"。

测试证据：改写/新增 4 项测试 RED→GREEN，`tests/unit/app + inline_domain` 192 passed（无回归）；E2E 复验（M626/ARRAY/11620 TDSUM）：趋势图去年份+柱图注+双轴正确，By Lot 值域 ~100-200 vs USL 618 不再全部超规，截图 `output/screenshots/aoi_tt_synced.png`。
