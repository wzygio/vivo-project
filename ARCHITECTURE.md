...
| `app/charts/<domain>/[<submodule>/]` | Chart adapters; Inline shared charts live in `app/charts/inline_domain/`, without a one-to-one directory for every backend submodule. AOI RS page rendering and PDF reports share UI-independent figure construction. Its presentation section owns the administrator-only product/factory export selection (both default to all) and application data consumption; its PDF adapter embeds three charts per indicator row and Chinese fonts, with temporary images cleaned on success and failure. Alert sections are excluded |
