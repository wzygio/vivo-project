...
def sanitize_to_compliant(spc_status_df, add_tag=True, **kwargs):
    """
    [企业级解耦版] 向量化蒙版引擎
    不再依赖上层 Service 的参数透传。直接从 ConfigLoader 获取规则。
    """
    ...
    # 2. 从统一配置中心动态拉取最新规则 (绝对安全，0 缓存报错)
    from src.shared_kernel.config import ConfigLoader
    rules = ConfigLoader.get_compliance_rules()
    
    # 3. 精准剔除保护名单 (动态向量化过滤)
    if rules and 'factory' in df.columns and 'prod_code' in df.columns:
        for rule_key, is_enabled in rules.items():
            # 只要 YAML 里配置了 false，就将这部分数据从“洗白名单”中剔除，予以真实展示
            if is_enabled is False: 
                parts = rule_key.split('-')  # 格式: AOI-M626-TP
                if len(parts) == 3:
