...
if mwd_group_data:
    ...
    with gc1:
        st.plotly_chart(create_group_trend_chart(df_m, "月度趋势", False, True, y_limit, COLOR_MAP, CATEGORY_ORDERS), use_container_width=True) # type: ignore
    with gc2:
        st.plotly_chart(create_group_trend_chart(df_w, "周度趋势", False, False, y_limit, COLOR_MAP, CATEGORY_ORDERS), use_container_width=True) # type: ignore
    with gc3:
        st.plotly_chart(create_group_trend_chart(df_d, "日度趋势", True, False, y_limit, COLOR_MAP, CATEGORY_ORDERS), use_container_width=True) # type: ignore
