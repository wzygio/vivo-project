...
from src.yield_domain.core.mwd_trend.code_baseline import (
    ...
    build_code_baseline_lookup as _build_code_baseline_lookup,
    code_baseline_path as _code_baseline_path,
    defect_multipliers_signature as _defect_multipliers_signature,
...
def _calc_code_ema_noise(
    ...
    """Code 级 EMA 计算 + 噪声注入 (日历网格化 + 按月分片隔离 + 锚点注入版)"""
    if raw_df.empty:
        return raw_df.copy()
    ...
    for code in unique_codes:
        if code == "NoDefect":
            no_def_mask = ema_df['defect_desc'] == 'NoDefect'
            result_frames.append(ema_df[no_def_mask].copy())
            continue
        ...
        full_dates = pd.date_range(start=start_month, end=max_date, freq='D')
        
        date_df = pd.DataFrame({'warehousing_time': full_dates})
        sub = pd.merge(date_df, sub, on='warehousing_time', how='left')
        sub['defect_desc'] = code
        defect_group_candidates = ema_df.loc[mask, 'defect_group'].dropna()
        defect_group_val = defect_group_candidates.iloc[0] if not defect_group_candidates.empty else 'Unknown'
        sub['defect_group'] = sub['defect_group'].fillna(defect_group_val)
        ...
        for month in months:
            month_sub = sub[sub['year_month'] == month].sort_values('warehousing_time').copy()
            counts = month_sub['defect_panel_count'].values
            ...
            smooth_all.extend(smooth)
        
        sub = sub.sort_values('warehousing_time').copy()
        sub['attenuated_rate'] = np.array(smooth_all) * scale
        sub['defect_panel_count'] = np.round(sub['attenuated_rate'] * sub['total_panels']).astype(int)
        
        result_frames.append(sub.drop(columns=['year_month'], errors='ignore'))
    
    new_ema_df = pd.concat(result_frames, ignore_index=True)
    return _inject_deterministic_noise_code_level(new_ema_df, volatility)
...
def _calculate_adaptive_shadow_ema(counts, totals, span, use_global_init=True, initial_anchor_rate=None):
    """
    自适应 EMA 计算函数 (V3.0 - 稳定锚点版)
    :param span: 平滑窗口大小
    :param use_global_init: 是否使用全局均值初始化 (当 initial_anchor_rate 未提供时生效)
    :param initial_anchor_rate: 外部注入的初始锚点率。如果提供，优先使用此值作为初始动量。
    """
    n = len(counts)
    if n == 0: return []
    alpha = 2/(span+1)
    ...
    if initial_anchor_rate is not None:
        # [新] 外部锚点注入模式
        t_n = t_d * initial_anchor_rate
        if totals[0] > 0:
            res.append(0.5 * initial_anchor_rate + 0.5 * first_rate)
        else:
            # 若当月 1 号无产出，EMA 直接输出该基准值（不动量衰减）
            res.append(initial_anchor_rate)
    elif use_global_init: # 使用平均良损计算初始动量
    ...
    for i in range(1, n):
        ...
        if t_d > 0:
            ...
            # 若前几日无产出导致动量分母为0，回退到初始锚点或全局均值
            p_base = initial_anchor_rate if initial_anchor_rate is not None else (base if use_global_init else 0)
