# 🎯 Target File: src/inline_domain/application/monitor/monitor_service.py
...
class MonitorAnalysisService:
    ...
    # =========================================================================
    @staticmethod
    def discover_monitor_products(data_root: Path) -> List[str]:
        """
        ...
        try:
            ...
            for data_dir in sorted(data_root.iterdir(), key=lambda x: x.name):
                ...
                if data_dir.name.startswith(('.', '__')):
                    continue
                if data_dir.name in MonitorAnalysisService.SPC_DATA_IGNORE_DIRS:
                    continue
    ...
    def fetch_dashboard_data_dict(
        ...
        try:
            ...
            config_instance.data_type_filter = "ALL"
        except Exception as e:
        ...
        if target_prod.upper() == "ALL":
            search_prods = MonitorAnalysisService.discover_monitor_products(data_root)
        else:
        ...
        for prod in search_prods:
            ...
            if data_type_upper in ('报废', 'ALL'):
                ...
                if not scrap_df.empty:
                    ...
                    trace_logger.info(f"🚧 [ScrapTrace][L3-TimeFilter] 时间范围: {start_dt} ~ {end_dt}, sheet_start_time 样本: {scrap_df['sheet_start_time'].head(3).tolist()}")
                    
                    mask = (scrap_df['sheet_start_time'] >= start_dt) & (scrap_df['sheet_start_time'] <= end_dt)
                    trace_logger.info(f"🚧 [ScrapTrace][L3-TimeFilter] mask 结果: {mask.tolist()}, 通过数: {mask.sum()}")
                    ...
                    if not scrap_df.empty:
                        ...
                        trace_logger.info(f"🚧 [ScrapTrace][L5-Append] 产品 {prod} 报废数据已加入 all_status_dfs")
                    else:
        ...
        if 'sheet_start_time' in raw_status_df.columns:
            ...
            trace_logger.info(f"🚧 [ScrapTrace][L8-TimeTrim] 时间边界过滤后: {len(raw_status_df)} 条, three_month_start={three_month_start}")
        ...
        trace_logger.info(f"🚧 [ScrapTrace][L9-StationAgg] station_detail_df: {len(station_detail_df)} 条, 列: {station_detail_df.columns.tolist()}")
        ...
        # =========================================================================
        full_status_df = MonitorAnalysisService._apply_time_bucket_mapping(raw_status_df, time_type.upper(), end_dt)
        trace_logger.info(f"🚧 [ScrapTrace][L10-TimeBucket] _apply_time_bucket_mapping 后: {len(full_status_df)} 条")
        ...
        trace_logger.info(f"🚧 [ScrapTrace][L11-DetailAgg] detail_df: {len(detail_df)} 条") 
    ...
    def get_monitor_dashboard_data(
        ...
        return MonitorDashboardViewModel(
