...
- The portal has a custom navigation source in `resources/static/config.js`. Its current CTQ item still points to the legacy FineReport URL, so automatic Streamlit page discovery alone would not make the new report reachable from the portal.
