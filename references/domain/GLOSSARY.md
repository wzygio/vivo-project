...
- **SPC 周期能力**：CPK/CPM 保留月、周趋势计算；修饰表只新增报表参考日期的上一完整 ISO 周中能力值 `< 1.33` 的异常，不新增本周、月度、达标或无法计算的记录。参考日期取查询截止日，不取最后一笔过货日。已有异常的人工值与 flag 保留。日度只保留趋势图需要的均值、标准差，不计算能力值，也不应用历史日度修饰。
- **SPC 能力替换值**：`cpk_replacement` / `cpm_replacement` 为 `flag=True` 时实际用于能力表和汇总指标的值，要求有限且严格介于 1.33 与 1.40；缺失或越界时随机生成 1.331–1.399 的三位小数，写入配置后复用，不随刷新重抽。`*_corrected` 保留历史记录值，`flag=False` 仍展示当前计算值。修饰只作用于匹配周期，不改变点位或其他周期；已启用记录不得因修饰后达标被清理。
