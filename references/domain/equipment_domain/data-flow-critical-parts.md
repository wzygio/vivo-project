...
## CVD 工作簿接入（2026-09-22）

- 资源入口为 `config/domain/equipment_domain.yaml` 的 `resources.files.cvd_parts`，当前指向 `供应商关键备件寿命管控清单-0921-CVD.xlsx`。仅消费首个 sheet；本次源表表头位于第 7 行、第 C 列起，包含 215 条记录、7 种备件。
- Infrastructure 的 `cvd_parts_loader.py` 定位字段表头，并将“机台号”等原始字段交给 Core。企业加密工作簿通过 `fr_file_decryption` 的 `excel-values` 策略读取；临时值副本放在 `output/decrypted_files/`，读取完成后清理，源文件不改写。该策略会丢失日期格式，因此 Core 按“年”规格识别并还原 Excel 日期序号。
- Core 的 `cvd_parts.py` 独立实现计算。188 条年限规格（2年、3年、5年）保留更换日期为 `YYYY-MM-DD` 测量值；使用进度为已使用天数 / 从更换日到额定周年到期日的天数，支持闰年。更换日期不随模拟推进。
- 27 条 Mask 数值记录按 `源测量值 + 进度 / 30 × 经过天数` 计算。起点优先取该行测量时间，空值使用 `equipment.cvd.baseline_date`，本次为 `2026-09-21`；早于起点时不倒扣。重复刷新从源值重新计算，不重复累加。CVD 不应用原有数值修饰、截断或寿命回绕；状态使用现有预警阈值。
- 应用服务在原有基线匹配、数值修饰和状态计算完成后追加 CVD 行，统一统计。CVD 不进入数据库查询条件或真实/仿造快照，不改变既有快照签名和优先级。CVD 的测量时间表示本次计算的报告日期。
- 页面从合并结果生成厂别、设备类型、备件类型筛选项。混合日期/数值的测量值和混合年限/数值的寿命规格在展示边界转为文本，兼容 Streamlit/Arrow；纯数值列继续保留数字类型。源字段“进度”在 Core 输出时即被排除，前端表格及原生导出只接收公开列。
- L2 缓存键新增 CVD 路径与文件修改时间/大小签名，继续包含报告日期和运行配置签名；源表更新、跨日或基准日期调整会触发重新计算。缺文件、字段错误或读取失败时不缓存部分报表，页面显示业务错误提示，技术原因写入服务端日志。
