...
核验日期：2026-09-14。范围：当前工作区 `src/` 的生产事实读取、原始快照、
...
### 1.1 Streamlit 定时重启与内存缓存

启动入口保留原有任务计划触发时间与运行账户。VBS 隐藏窗口并等待启动检查结束，
将 BAT/PowerShell 的退出码传回任务计划：健康检查成功为 `0`，失败为非零。
它不等待整个 Streamlit 服务生命周期，因此成功启动后任务可以结束，服务继续运行。

[重启脚本](../../../tools/restart_streamlit.ps1) 默认操作 8503 端口，流程如下：

1. 获取跨进程重启锁，检查项目 `.venv`、入口文件及 Streamlit 是否可用。
2. 用 Streamlit 启动参数、项目入口/虚拟环境路径和精确端口识别旧进程，兼容原有
   `uv run streamlit` 启动的解释器链。端口被未确认属于本项目的进程占用时拒绝重启。
3. 复核 PID 与创建时间，结束识别出的旧进程并等待退出；发现残留进程或监听端口
   时返回失败，不继续启动。
4. 使用项目 `.venv/Scripts/python.exe -m streamlit` 隐藏启动，核实监听进程属于
   本次启动，并等待 `/_stcore/health` 返回 HTTP 200，默认启动检查期限为 60 秒。
5. 将开始/结束时间、旧 PID、新 PID 和成功/失败结果写入
   `output/logs/streamlit-startup/` 的每次运行 JSON，服务输出分别写入 stdout/stderr
   日志。新服务启动检查失败时尝试清理该次启动的进程，并报告清理失败。

每次成功重启都建立新的 Python 进程，原有 `st.cache_data` 内存数据、
`st.cache_resource` 资源与会话状态随旧进程退出而释放，后续页面访问重新建立。
磁盘原始快照、修饰工作簿与矩阵状态文件继续遵循各自的持久化和 TTL 规则。
健康接口验证服务已就绪，不会为所有页面执行查询或预热缓存。

运维命令 `powershell -File tools/restart_streamlit.ps1` 与 BAT 使用同一流程；
日常计划任务仍使用 `run_hidden.vbs`。脚本不在启动时自动同步或升级依赖，
依赖更新由部署时的 `uv sync` 完成。
...
`application.incremental_refresh_days` 为正整数，缺省时使用 7；配置非法时拒绝使用。
它表示从各模块原有增量截止点向前减去的天数，再查询至本次请求末日；不是从
服务器今天倒推固定 7 天。增量起点仍受当前源窗口起点限制。配置在下次实际
增量刷新时读取；TTL 内的有效快照仍可复用，显式全窗口刷新维持原有行为。
...
| 模块 | 快照粒度 | 普通刷新 | 显式刷新 |
|---|---|---|---|
| Yield | 产品 + 静态数据策略签名 | 旧快照最新入库日期按全局天数回读（默认 7 天），补查至当前请求末日 | `force_refresh=True` 全请求窗口重取 |
| Inline 共享测量 | 每产品一份；SPC、CTQ、AOI_TT、Monitor 复用 | 覆盖截止日按全局天数回读（默认 7 天），替换区间 | `force_refresh=True` / `refresh_measurements()` 全滚动窗口重取 |
| AOI_RS | 每产品 RS 明细与过货分母两份 | 各自覆盖截止日按全局天数回读（默认 7 天），替换区间 | `force_refresh=True` 全窗口；`refresh()` 先取得两份全窗口数据，再成组发布 |
| Q-Time | 每厂别一份，供多个产品/路径复用 | 源覆盖结束边界按全局天数回读（默认 7 天），替换区间 | 页面按钮全窗口；CLI 默认增量、`--full` 全窗口 |
...
```text
查询起点 = max(本次源窗口起点, 旧快照最大 warehousing_time - incremental_refresh_days 天)
查询终点 = 本次源窗口终点
```
...
而不是只回读配置的重叠区间。发布保留源时间与下午数据，不要求预先删除生产快照。
...
增量起点 = max(当前原始窗口起点, covered_through - incremental_refresh_days 天)
```
...
共享测量和 AOI_RS 的强刷跳过 TTL 和上述增量起点，从请求结束日第三个前序自然月
的 1 日重取至请求结束日，不合并旧事实。例如结束日为 2026-09-14 时，原始查询
窗口为 2026-06-01 至 2026-09-14；页面常规结束日为今天。成功空查询也完整替换
旧窗口，因此历史修改、删除和清空均能同步。刷新仍按传入的产品独立执行，
不会遍历其它产品；普通读取遵循 TTL 与全局配置的回读天数。
AOI_RS `refresh()` 先取得明细与分母两份全窗口数据，再成组发布；任一取数失败
不发布另一份新结果。
...
```text
查询起点 = max(本次源窗口起点, min(旧 source_end, 本次 source_end) - incremental_refresh_days 天)
...
| Monitor 报废数据 | 从 Excel 读取、按产品过滤，再转换日期和截断；没有按快照截止日回读的逻辑 |
...
以原始日期和默认 7 天配置为例，Inline 快照已覆盖到 9 月 4 日，9 月 11 日再次读取：

```text
保留旧快照中 8 月 28 日之前的记录
从 8 月 28 日（9 月 4 日减 7 天）查询到 9 月 11 日
用查询结果替换该区间
更新完整覆盖元数据
最后按报表时间策略截断输出
```

9 月 5 日至 11 日均会补查，不受访问间隔长短影响。Q-Time 的
结束边界不包含次日零点：旧源覆盖到 `9 月 5 日 00:00` 时，下次从 `8 月 29 日
00:00` 回读至新的结束边界。Yield 使用旧记录的最大入库日期，若最大日期也是
9 月 4 日，则从 8 月 28 日补查；实际最大日期更早时可能查得更多。

这个结论依赖数据库查询成功、源事实仍存在且处于本次保留窗口。它不意味着系统
永久保存全部历史，也不保证发现已覆盖区间中很久以前的迟到补录：

- 上游修改发生在配置的重叠区间（默认 7 天）之外，普通增量通常不会重新核验那段历史。
- Yield 的追加去重不保证同步源删除；Inline/Q-Time 在回读区间内以区间替换处理。
- 刷新失败的旧数据回退不是补查完成，不能推进覆盖日期或据此宣称当前数据完整。
- Yield、Inline/AOI_RS 和 Q-Time 页面手动刷新均全窗口重取，可同步窗口内较早
  历史的修改；各领域窗口及产品/厂别粒度不同，不代表数据库全部历史或所有产品。
