...
| Inline 共享测量 | 每产品一份；SPC、CTQ、AOI_TT、Monitor 复用 | 覆盖截止日回读 2 天，替换区间 | `force_refresh=True` / `refresh_measurements()` 全滚动窗口重取 |
| AOI_RS | 每产品 RS 明细与过货分母两份 | 各自覆盖截止日回读 2 天，替换区间 | `force_refresh=True` 全窗口；`refresh()` 先取得两份全窗口数据，再成组发布 |
...
不会遍历其它产品；普通读取的 TTL 与两天回读增量机制保持不变。
