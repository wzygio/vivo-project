...
2026-09-22 页面拆分：`app/pages/自动预警看板.py` 仅保留全指标预警矩阵；
`app/pages/超规与CPK预警看板.py` 承载超规片与 CPK 两个独立查询区。
门户 `app/static/config.js` 的“自动预警”分支及侧边栏提供两个入口。
新页面不加载矩阵或检查其定时快照，避免全指标查询阻塞其他看板入口；
原有查询门控、数据口径和产品指标缓存机制保持不变。
...
页头“刷新缓存”还承担显式应用代码更新的职责：无论有无产品/指标作用域，
均执行后端模块卸载、产品配置重读、当前会话视图缓存及变更提示清理。
代码模块属于整个 Python 进程，重载不能按产品隔离；上述数据 revision
仍只推进选中的产品/指标，不因此调用全局 `st.cache_data.clear()`。
其他会话后续导入模块时使用新代码，已有数据缓存仍遵循自己的版本和依赖。
缓存载荷继续遵守 [原生载荷边界](../../../docs/ADR/0001-streamlit-cache-native-payload-boundary.md)，
避免模块重载导致项目类身份不一致。

2026-09-22 恢复记录：`f60dc5c`（2026-09-08）的缓存隔离改动曾在
`perform_hard_reset` 的 `indicator_product` 分支提前返回，跳过模块重载和
会话视图清理。按明确的代码重载要求移除此绕过；页头“刷新数据”在快照任务全部成功后，
也复用“刷新缓存”的完整流程：缓存失效、代码重载、配置重读及会话视图清理。
两种操作的顺序和失败边界以 [页头更新机制](../feat_design/data-flow-page-header-refresh.md) 为准。
