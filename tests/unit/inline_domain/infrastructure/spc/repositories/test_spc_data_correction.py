...
def test_correction_shifts_m673_ppa_sites_96_to_114() -> None:
    ...
    assert result.loc[2, "param_value"] == 9.5
    assert result.loc[3, "param_value"] == 9.5
    assert result.loc[4, "param_value"] == 10.0
    assert result.loc[5, "param_value"] == 9.5
    assert result.loc[6, "param_value"] == 9.5
...
def test_snapshot_repository_applies_corrector_before_writing_snapshot(tmp_path) -> None:
    ...
    assert snapshot.loc[2, "param_value"] == 9.5
