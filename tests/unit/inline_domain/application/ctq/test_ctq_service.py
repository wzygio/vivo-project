...
def test_ctq_service_loads_ctq_distributions_without_capability_fields(
    ...
    assert report.sheet_oos_decoration_result.decoration_path.parent == tmp_path / "resources" / "M678"
    assert not (tmp_path / "resources" / "M678" / "ctq_sheet_oos_detail.xlsx").exists()
    assert [
        path.name for path in (tmp_path / "resources" / "M678").glob("*.xlsx")
