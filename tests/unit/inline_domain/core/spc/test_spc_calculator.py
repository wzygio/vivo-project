...
def test_calculate_cpm_equals_cp_at_spec_midpoint() -> None:
    cpm = calculate_cpm(mean_value=50.0, std_value=1.0, usl=55.0, lsl=45.0)

    assert cpm == 10.0 / 6.0


@pytest.mark.parametrize("mean_value", [8.0, 12.0])
def test_calculate_cpm_uses_image_midpoint_penalty(mean_value: float) -> None:
    cpm = calculate_cpm(mean_value=mean_value, std_value=1.0, usl=16.0, lsl=4.0)

    assert cpm == pytest.approx(1.5)


@pytest.mark.parametrize("mean_value", [10.0, 12.0, 20.0])
def test_calculate_cpm_zero_sigma_returns_infinity(mean_value: float) -> None:
    assert calculate_cpm(mean_value, 0.0, 16.0, 4.0) == float("inf")


def test_calculate_cpm_remains_positive_when_mean_exceeds_spec() -> None:
    assert calculate_cpm(18.0, 1.0, 16.0, 4.0) == pytest.approx(6.0 / 7.0)
    assert calculate_cpk(18.0, 1.0, 16.0, 4.0) == pytest.approx(-2.0 / 3.0)


def test_calculate_cpm_returns_nan_for_single_sided_specs() -> None:
    cpm = calculate_cpm(mean_value=50.0, std_value=1.0, usl=55.0, lsl=np.nan)

    assert math.isnan(cpm)


def test_calculate_cpm_returns_nan_when_zero_lsl_marks_an_upper_only_spec() -> None:
    cpm = calculate_cpm(mean_value=3.0, std_value=0.5, usl=8.0, lsl=0.0)
...
def test_cpk_and_cpm_share_the_same_invalid_input_gate() -> None:
    ...
    cpm = calculate_cpm(mean_value=3.0, std_value=-0.5, usl=8.0, lsl=1.0)
