...
from app.sections.inline_domain.shared import decoration_admin, sheet_oos_admin
...
def test_download_bytes_do_not_contain_refresh_meta(tmp_path: Path) -> None:
    ...
    data = decoration_admin.excel_bytes(build_decision_download_sheets(result))
...
def _patch_admin_st(monkeypatch, upload: _FakeUpload | None, messages: dict) -> None:
    ...
    monkeypatch.setattr(decoration_admin.st, "expander", lambda *_a, **_k: nullcontext())
    monkeypatch.setattr(decoration_admin.st, "caption", lambda *_a, **_k: None)
    monkeypatch.setattr(decoration_admin.st, "markdown", lambda *_a, **_k: None)
    monkeypatch.setattr(decoration_admin.st, "columns", lambda *_a, **_k: [FakeColumn(), FakeColumn()])
    monkeypatch.setattr(decoration_admin.st, "download_button", lambda *_a, **_k: None)
    monkeypatch.setattr(decoration_admin.st, "file_uploader", lambda *_a, **_k: upload)
    monkeypatch.setattr(decoration_admin.st, "button", lambda *_a, **_k: True)
    monkeypatch.setattr(decoration_admin.st, "success", messages["success"].append)
    monkeypatch.setattr(decoration_admin.st, "error", messages["error"].append)
    monkeypatch.setattr(decoration_admin.st, "info", messages["info"].append)
    monkeypatch.setattr(
        decoration_admin.st, "rerun", lambda: messages["rerun"].append(True)
    )
...
def test_cached_payload_decisions_flow_to_download_and_unchanged_upload(
    ...
    # master 域资源路由：修饰工作簿位于 resources/inline_domain/ 下
