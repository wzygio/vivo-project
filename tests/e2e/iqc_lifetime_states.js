async page => {
  ...
  const mainGroups = page.getByTestId('stExpander').filter({ has: page.locator('summary', { hasText: 'M678' }) });
  if (await mainGroups.count() !== 2) throw new Error('Expected separate metric expanders');
  ...
  return { status: 'passed', scenarios: ['failure', 'empty', 'recovery', 'missing', 'nine-samples', 'complete-native-export', 'filter-reset', 'anonymous-hover', 'enabled-products', 'product-status', 'two-metrics', 'four-charts-per-row', 'independent-expanders', 'matching-sample-colors'] };
}
