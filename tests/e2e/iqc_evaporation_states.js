async page => {
  ...
  await page.goto('http://localhost:8518/?scenario=empty');
  await page.getByRole('button', { name: '查询', exact: true }).click();
  ...
  if (await page.locator('[data-testid="stMultiSelect"]').count() !== 0) throw new Error('Extra filters remain');
  if (await page.locator('[data-testid="stException"]').count()) throw new Error('Streamlit exception');
  return { status: 'passed', source: 'controlled data port', scenarios: ['failure', 'recovery', 'full-result', 'date-only', 'empty'] };
}
