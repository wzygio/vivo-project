"""Self-contained indicator PDF layout: two indicator rows per landscape A3 page."""
...
ROW_HEIGHT = 340
FONT_NAME = "indicator-report-font"
INK = (0.18, 0.24, 0.33)
MUTED = (0.42, 0.47, 0.54)


class IndicatorPdfReport:
    ...
    def append_indicator(
        ...
        if len(images) != 3:
            raise ValueError("Each report row must contain exactly three images")
    ...
    def to_bytes(self) -> bytes:
        for page in self.document:
            page.insert_text(
                ...
                f"{self.report_name}    |    {page.number + 1} / {len(self.document)}",
                fontname=FONT_NAME,
                fontsize=10,
                color=MUTED,
            )
        self.document.set_metadata({"title": f"{self.report_name}监控报表", "subject": self.period})
