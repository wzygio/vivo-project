"""Shared, UI-independent AOI RS figure construction for the page and exports."""
...
def _code_spec_map(
    indicators_df: pd.DataFrame, spec_df: pd.DataFrame, chart_kind: str
) -> dict[str, float | None]:
...
class ChartGroup:
    ...
    code: str
    code_name: str
    figures: tuple[tuple[str, go.Figure], ...]


def iter_aoi_rs_chart_groups(
    *,
    ...
    """Yield one set of three charts per indicator, with the page's data semantics."""
                ...
                (
                    (
                        ...
                        create_aoi_rs_trend_chart(
                            trend_df=step_trend[
                                step_trend["rs_code"].astype(str) == code
                            ],
                    ...
                    (
                        ...
                        create_aoi_rs_point_chart(
                            point_df=step_sheet[
                                step_sheet["rs_code"].astype(str) == code
                            ],
                            id_col="sheet_id",
