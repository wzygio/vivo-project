...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fabricate one current measurement per monitorable critical-parts key.",
    )
    parser.add_argument(
        "--baseline",
        type=Path,
    ...
    parser.add_argument(
        "--as-of",
        help="ISO timestamp recorded in every fabricated row; defaults to the current local time.",
    )
    ...
    result = fabricate_current_snapshot(spec_df, policy, as_of=as_of)
