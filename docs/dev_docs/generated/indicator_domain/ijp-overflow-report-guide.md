...
5. 先看 `OLED RS Overflow By天` 堆叠图判断 CODE 构成变化，再从明细表定位 Glass、Panel、设备和边框位置。
...
图表按“产品 → 线体 → Printer”分层展示，每个 Printer 独立生成一幅“日期 × RS_CODE”100% 堆叠柱图：

- 每个产品对应一个外层 Expander，每条线体对应一个子 Expander；
- 每个线体下的 Printer 图横向排列，每行最多三幅；
- 每个颜色代表一个 RS_CODE；
- 每个 Printer 每天的各 CODE 占比合计约为 100%；
- 占比计算为：`该产品、线体、Printer 当日某 CODE 记录数 ÷ 同组当日全部受监控 CODE 记录数`；
...
- 返回的 `Print Time` 和图表日期再按相同偏移量映射为显示时间；
- 原始数据库事实时间不会被修改。
