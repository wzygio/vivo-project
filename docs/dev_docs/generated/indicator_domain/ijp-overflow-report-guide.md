...
> 文档依据：当前仓库源码、ADR-0020/0021/0022、IJP 数据源分析；核对日期：2026-09-07。
...
1. 打开“**IJP溢流监控报表**”页面。
2. 确认页面显示的数据范围。系统固定查询上个月 1 日 00:00:00 至今天 23:59:59.999999，用户无需选择时间。
3. 按需要选择产品型号、线体、CODE 和批次。
4. 点击“**查询**”。仅修改筛选条件不会自动查询；条件改变后，旧结果会失效，需要再次点击“查询”。
5. 先看 `OLED RS Overflow By Glass ID` 堆叠图判断各片玻璃的 CODE 构成，再从明细表定位 Glass、Panel、设备和边框位置。
...
### 4.1 `OLED RS Overflow By Glass ID` 图

图表按“产品 → 线体 → Printer”分层展示，每个 Printer 独立生成一幅“Glass ID × RS_CODE”100% 堆叠柱图。横轴按 Glass ID 排序，一根柱对应一片玻璃，同一天的不同玻璃分别展示：

- 每个产品对应一个外层 Expander，每条线体对应一个子 Expander，两级默认展开；
- 每个线体下的 Printer 图横向排列，每行最多三幅，不足三幅时均分整行；
- 每个颜色代表一个 RS_CODE；
- 每个 Printer 内每片 Glass 的各 CODE 占比合计约为 100%；
- 占比计算为：`该产品、线体、Printer、Glass ID 的某 CODE 记录数 ÷ 同组全部受监控 CODE 记录数`；
- 它反映的是**缺陷代码构成**，不是良率、不良率、Glass 异常率或设备溢流率；
- 某个颜色占比更高，表示该 CODE 在该片玻璃受监控记录中的相对份额更高，不一定表示其绝对数量更多。

图表和明细使用同一固定时间范围，均从上个月 1 日开始，不再为趋势图额外向前扩展 7 天。
...
明细末尾的 `Total` 是当前展示行中 `CODE_RATIO` 数值的直接求和。由于同一 Glass/CODE 的比例可能在多条缺陷明细中重复，而且 BOTTOM 记录还可能展开出细分行，所以该 Total **不是总体溢流率，也不要求等于 1 或 100%**。判断代码构成优先使用上方 By Glass ID 图；判断单片 Glass 时，应结合该 Glass 的全部明细查看。

## 5. 时间口径

页面查询范围固定为显示时间的“上个月 1 日至今天”。制造事实的源时间与显示时间仍由 `config/global.yaml` 的 `data_forward` 控制：

- 页面输入和输出都是**显示时间**；
- 启用非零偏移时，系统查询数据库会自动把显示窗口反算为源时间窗口；
- 返回的 `Print Time` 按相同偏移量映射为显示时间，图表横轴 Glass ID 不进行时间偏移；
- 原始数据库事实时间不会被修改。
...
1. 先在固定时间范围内用较宽条件查看各 CODE 的日构成是否突变；
2. 再按产品、线体或批次逐步缩小范围；
3. 用 CODE 定位到具体记录；
