...
页面固定统计上个月 1 日至今天，不再提供日期选择。每个产品、线体、Printer 下展示五根 CODE 百分比堆叠柱：上月、本月、最近三周（含本周，周一至周日；本周截至今天）。周跨范围边界时只使用固定窗口内数据。月、周是分别汇总的两组视角，同一记录可同时计入所属月份和周，五柱不可相加。

各周期按原始 CODE 记录数加权，不平均 Glass 百分比；每个有数据周期分别应用既有 90%～92% 稳定随机目标修饰，原始比例通过悬停查看。无数据周期保留位置并标注“无数据”，不生成虚假的 C3DM1 占比。明细和历史绘图函数保留，但页面不展示。
...
以下章节中关于日期选择和六根独立 CODE 柱的描述属于历史版本；当前操作和图形以本节为准。
...
5. 查看机台 CODE 六柱图；悬停可核对实际占比和记录数。
6. 当前页面暂不查询、展示明细表，原明细构建与渲染函数保留供后续恢复。
...
- C3DM1 的目标值在 `ijp.c3dm1_minimum`～`ijp.c3dm1_maximum`（默认 0.90～0.92）间稳定随机生成。种子依据产品、线体、Printer 与各 CODE 汇总记录数，相同数据重复查询不跳动，输入行顺序不影响结果。实际值不足目标时提升到目标，其它 CODE 按 `(1-目标)/(1-C3DM1实际占比)` 同比例缩放；实际值达到目标则保留。排除 C3DM1 时不修饰；无数据不生成虚假机台。
