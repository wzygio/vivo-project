...
本文描述当前代码的实际行为，不展开正则匹配、随机数和数值修饰的详细算法。
...
核心原则是：**规格基线决定监控项，真实快照优先，独立伪造快照只补真实匹配缺口；报表读取不隐式生成或更新伪造数据。**
...
首次生成必须显式运行 `tools/fabricate_equipment_data.py`。机制如下：
...
7. 固定随机种子和基准时间时结果可复现。
8. 默认拒绝覆盖已存在的目标文件；只有显式 `--overwrite` 才允许替换。

### 6.2 后续更新

后续推进必须显式运行 `tools/update_fabricated_equipment_data.py`。它只更新已有文件，不会在文件缺失时自动执行首次生成：

1. 通过当前规格签名定位伪造快照；文件不存在立即报错。
2. 用文件修改时间计算年龄。小于伪造 TTL（当前 24 小时）时跳过更新。
3. `--force` 只绕过新鲜度检查，不绕过结构、键和数值校验。
4. 更新前校验五个必需列、非空数据、唯一键、合法数值/时间，以及每个快照键仍能映射回当前规格。
5. 每次有效更新将每条测量时间推进 1 天，将测量值按配置增量推进；当前增量为寿命规格的 30%。
6. 推进后的值超过寿命规格时，按配置重置到寿命规格的 0%～30%。
7. 更新保持原有键集合和行数，并原位覆盖同一伪造快照文件。
...
- 缺失时返回一个具有标准列的空 DataFrame；
- 结构缺列时直接报错；
- 页面访问不会创建、修复或推进伪造文件；
- 页面上的“刷新数据”也只刷新真实数据库快照，不更新伪造快照。
...
4. 对所有过高的展示值再执行展示上限处理。当前上限为寿命规格的 96%，避免进度组件显示成 100%。
...
需要特别注意：最终 `预警状态` 基于修饰后的展示值，不基于 `是否超规` 原始标记。按当前 96% 展示上限，原始超规记录会保留 `是否超规=True`，但展示进度最多为 96%，最终通常显示为“预警”而不是“超规”。因此页面“超规”指标表达的是修饰后状态，不是原始超规数量。
...
| 伪造文件未满 24 小时 | 更新命令跳过，除非显式 `--force` |
