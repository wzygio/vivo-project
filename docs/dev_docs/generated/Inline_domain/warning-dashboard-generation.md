...
`resources/inline_domain/monitor/compliance_config.xlsx` 的 `全指标预警看板` sheet
按页面的监控参数行、产品列排列，数据单元格默认为布尔 `False`。
`True` 将对应产品×指标显示为达标，点击也只显示达标说明，不加载原始预警详情；
`False` 恢复原始状态（后台 `no_data` 仍按既有前端口径显示为达标）。
此开关跨周、跨厂别筛选持续生效，直到用户设回 `False`；不要改动行名和产品列名。

保存 Excel 后点击矩阵上方“更新显示”（或普通页面重运行），无需点击“刷新缓存”。
只刷新按文件修改时间与大小缓存的小型配置；原始矩阵、详情及底层计算缓存不清除。
表格和详情共享缓存外的显示副本，不写回缓存 payload，也不修改源明细和汇总 Excel。
文件损坏、缺 sheet 或非法开关会提示配置未生效，保留原始状态；不静默覆盖为达标。
历史厂别／产品／类型／月份四维修饰已退出后台运行链路，旧管理面板和上传清缓存逻辑已移除。
