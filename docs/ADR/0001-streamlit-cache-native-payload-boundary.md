...
- Yield 另将分析起止日期、修饰表、警戒线和 rate override 文件签名写入缓存键；
  ...
  2026-09-21：入库不良率分析看板按用户要求采用手动资源版本，普通交互不再
  检测上述文件的修改时间；页头指标版本控制缓存失效，Mapping 配置也按产品与
  该版本复用。分析日期、全局 TTL 和时间策略仍生效；TTL 到期、缓存淘汰或进程
  重启后的首次计算仍读取当前资源。其他 Yield 调用方保留文件签名模式。
